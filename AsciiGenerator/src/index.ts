...
import { loadImageNode, resizeImage, CanvasImageData } from './imageToAscii.js';
...
async function generateAsciiFiles() {
    try {
        console.log(`Loading image from ${imagePath}`);
        const imgCanvas = await loadImageNode(imagePath);
...
function generateAsciiAndColors(imageData: CanvasImageData, color: boolean = true, isRight: boolean = false): { asciiArt: string, colors: Set<string> } {
...
function generateCss(colors: Set<string>): string {
    let css = '';
